...
- **Never type secrets yourself — use `request-fill` ("Keeper")** — for passwords, login codes, 2FA, or any value you must not see, call `POST /api/sessions/{id}/request-fill` instead of `fill`/`type`. **This is THIS service's own built-in secure credential fill — "Keeper" is the user's companion app for it. It is NOT OpenClaw `secrets`/`SecretRefs` (config-backed values); do not conclude "there's no keeper" because OpenClaw secrets has no live prompt — this endpoint IS the live prompt.** The user supplies the value out-of-band in their Keeper app (which shows your `message` + a screenshot of the field area) and the **service** types it into the field; the value is never returned to you or logged. It's **async**: you get `{request_id, status:"pending"}`, then poll `GET /api/sessions/fill-status/{request_id}` (tool `get_fill_status`) until `filled` / `cancelled` / `timeout` / `error`.
  ...
  - **Generate a NEW password (`generate`)** — for a sign-up or change-password flow, set **`generate":true`** on the field and the **Keeper creates a strong value itself, fills it, and saves it — UNATTENDED, no user prompt**; you just poll to `filled` and never see the value. Policy: `>= 14` chars with lower/upper/digit guaranteed; use **`length`** to make it longer (values below 14 are raised), and **`symbols":false`** to exclude symbols (sites that reject them). Numeric PIN: `{"generate":true,"format":"numeric","length":6}`. Example: `{"selector":"#new-pass","label":"New password","field":"password","generate":true,"message":"Creating your account"}`. To have the USER pick the password, just omit `generate` (that prompts as normal). **Sign-up forms: send the password AND confirm-password selectors in ONE request_fill** — every `generate` field in a single request gets the SAME value, so the confirmation matches; two separate calls generate two different passwords and the form rejects the submit. (A user may flip a per-device Keeper Setting to review generated values first — transparent to you.)
