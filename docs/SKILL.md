...
- **Stored sessions** — Sessions persist to S3 and are restored on reopen. The **whole browser profile** is saved/restored as one consistent unit (cookies, localStorage, sessionStorage, IndexedDB, **Service Workers**, Cache Storage, metadata), so apps that keep their login in IndexedDB/Service Workers come back **logged in**. List with `GET /api/stored-sessions`, then reopen by creating a session with the same `session_id`. If `url` is not provided on connect, the saved page URL is used for redirect. Use `GET/PUT /api/stored-sessions/{session_id}` to read or update metadata (e.g. redirect URL); `DELETE` wipes all persisted state. Connecting/disconnecting (incl. CDP) does not destroy a session — only idle timeout or explicit `DELETE` does.
